...
6. Return the child skill's completed draft without adding a second analysis. If the early production contract delegates the whole task and the user has not reserved script review, freeze it as `script_source: "planned_under_end_to_end_delegation"`, record the delegation rather than a fictitious user approval, and continue. Otherwise stop for the user's explicit script approval. Resolve only real conflicting or missing inputs; never re-ask the already selected TTS/edit scope.

If the user edits or rejects the draft, honor that feedback and any requested review before proceeding. Record `script_source: "planned_and_user_approved"` only after actual script approval; retain `planned_under_end_to_end_delegation` for an automatically prepared draft. Resolve the product-detail page URL by the existing-context reuse rule below. Reuse the early ratio, scope, voice, and category answers; ask only for actual missing or conflicting inputs. Continue to the product gate only after both the URL and ratio are present.
